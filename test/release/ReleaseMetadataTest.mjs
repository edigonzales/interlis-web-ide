...
test("checks the committed Web IDE contract without sibling tarballs", async () => {
  ...
  assert.equal(lock.dependencies.languageTools.legacyWithoutDependencyLock, true);
});
...
test("renders local tarball overrides for an explicit Language-Tools checkout", () => {
  const template = 'overrides:\n  "@ilic/tools": "file:../interlis-language-tools/artifacts/npm/ilic-tools-snapshot.tgz"\n';
  assert.equal(
    renderLocalWorkspace(template, "/work/web", "/tmp/upstreams/language"),
    'overrides:\n  "@ilic/tools": "file:../../tmp/upstreams/language/artifacts/npm/ilic-tools-snapshot.tgz"\n',
  );
});
