...
export class WebIdeWorkbench {
  ...
  async #requestCloseTab(tab: OpenTab): Promise<boolean> {
    if (tab.dirty && !tab.readOnly) {
      ...
      } else await this.#recovery.clear(tab.model.uri.toString());
    }
  ...
  async #renderExplorer(host: HTMLElement): Promise<void> {
    ...
    count.textContent = String(this.#tabs.size);
  ...
  #renderOpenEditors(
    ...
    if (heading) {
      ...
      if (count) count.textContent = String(this.#tabs.size);
    }
  ...
  #scheduleAutoSave(tab: OpenTab): void {
    ...
    tab.autoSaveTimer = setTimeout(() => {
      tab.autoSaveTimer = null;
      void this.#saveTab(tab, true).catch((error: unknown) =>
        this.logError(`Auto-save ${tab.label}`, error),
      );
    }, autoSaveDelay);
  }
